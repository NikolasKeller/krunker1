...
import { neutralInput, validInput } from '../src/shared/movement';
import { wirePlayer, playerDelta } from '../src/shared/snapshot';
import { reconcile } from '../src/client/prediction';
...
test('binary full/delta snapshots reconstruct every player field and preserve long-running acknowledgements', () => {
    ...
    const decoded = decodeServerMessage(encodeServerMessage(full)); assert.ok(decoded.type === 'snapshot');
    ...
    for (const key of Object.keys(a) as (keyof PlayerState)[]) {
        ...
        if (typeof expected === 'number') assert.ok(Math.abs((got as number) - expected) < .00001, key);
    ...
    const delta = decodeServerMessage(encodeServerMessage({ ...full, n: 43, base: 42, full: false, players: [patch], removed: ['gone'], round: undefined }));
    ...
    assert.ok(Math.hypot(reference.x - binary.x, reference.y - binary.y, reference.z - binary.z) < .001);
});
