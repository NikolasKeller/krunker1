"""Measure the deployed combat-feedback candidate from inside Railway, using Node only.

After the patch is committed and deployed:
  python3 tests/railway-combat.py --expect-revision <full candidate SHA>
...
remote = '/tmp/arena-combat-verification'
if args.collect:
    ...
    for line in output.splitlines():
        ...
        path = root / ('artifacts/combat-feedback/railway-' + item['name'] + '.json')
...
for name, source in [('load', 'tests/load.ts'), ('websocket', 'tests/bad-link-websocket.ts')]:
    target = pathlib.Path('/tmp/arena-combat-' + name + '.mjs')
...
print(ssh(launcher))
print('Allow about six minutes, then run: python3 tests/railway-combat.py --collect')
