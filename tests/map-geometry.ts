...
import { BOXES, MAP_SIZE, SOLID_BOXES, RAMPS, type MapBox } from '../src/shared/map';

export const BOUNDS_TOLERANCE = 1e-4;
export function headlessMap(batch = false) {
    ...
    try {
        ...
        const objects = buildMap(scene, { batch });
...
export function buildingBounds(objects: MapObject[]) {
    return BOXES.filter(b => b.kind === 'building').map(building => {
...
export function auditMap(scene: THREE.Scene, objects: MapObject[]) {
    scene.updateMatrixWorld(true);
