...
export function installDOM(url = 'http://localhost:8080') {
    ...
    const values = { window: dom.window, document: dom.window.document, localStorage: dom.window.localStorage, location: dom.window.location, history: dom.window.history, sessionStorage: dom.window.sessionStorage, addEventListener: dom.window.addEventListener.bind(dom.window) };
