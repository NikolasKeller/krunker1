...
export function validInput(v:unknown):v is Input {
  if(!v||typeof v!=='object')return false;const i=v as Input;
...
export function move(p:MoveState,i:Input,speedScale=1,dt=STEP):void {
  p.slideAge+=dt;
  if(i.jump&&!p.jumpHeld)p.jumpBuffer=0.12;else p.jumpBuffer=Math.max(0,p.jumpBuffer-dt);
  if(i.slide&&!p.slideHeld)p.slideAge=0;
  p.jumpHeld=i.jump;p.slideHeld=i.slide;
  if(p.grounded){p.coyote=0.07;p.groundTime+=dt;}else{p.coyote=Math.max(0,p.coyote-dt);p.groundTime=0;}
  let speed=Math.hypot(p.vx,p.vz);
  if(p.grounded&&i.slide&&p.slide<=0&&p.slideAge<0.25&&speed>5){p.slide=0.65;const boost=p.slideAge<0.13?1.16:1.09;p.vx*=boost;p.vz*=boost;}
  if(p.jumpBuffer>0&&(p.grounded||p.coyote>0)) {
    const timed=p.groundTime<0.13&&speed>5;
    if(timed){p.vx*=p.slide>0?1.105:1.035;p.vz*=p.slide>0?1.105:1.035;}
    p.vy=JUMP_SPEED*(i.aim?0.94:1);p.grounded=false;p.coyote=0;p.jumpBuffer=0;p.slide=0;
  }
  p.slide=Math.max(0,p.slide-dt);if(!i.slide)p.slide=0;
  ...
  for(const axis of ['x','z'] as const) {
    const velocity=axis==='x'?'vx':'vz';p[axis]+=p[velocity]*dt;
    for(const b of BOXES) {
      if(p.y>=b.y+b.h/2-0.035||p.y+bodyH<=b.y-b.h/2+0.02)continue;
      if(Math.abs(p.x-b.x)<b.w/2+RADIUS&&Math.abs(p.z-b.z)<b.d/2+RADIUS) {
        const top=b.y+b.h/2;
        if(top-p.y<=0.34&&p.grounded){p.y=top;continue;}
        const center=axis==='x'?b.x:b.z,half=axis==='x'?b.w/2:b.d/2;
        p[axis]=center+(p[axis]<center?-1:1)*(half+RADIUS);p[velocity]=0;
      }
    }
    for(const r of RAMPS) {const h=rampHeight(r,p.x,p.z);if(h!==null&&h>p.y+0.62&&p.y<r.h) {p[axis]-=p[velocity]*dt;p[velocity]=0;}}
  }
  ...
  for(const b of BOXES) {
    if(Math.abs(p.x-b.x)<b.w/2+RADIUS*0.65&&Math.abs(p.z-b.z)<b.d/2+RADIUS*0.65) {
      ...
      const bottom=b.y-b.h/2;if(p.vy>0&&oldY+bodyH<=bottom&&p.y+bodyH>=bottom){p.y=bottom-bodyH;p.vy=0;}
    }
  }
  for(const r of RAMPS) {const h=rampHeight(r,p.x,p.z);if(h!==null&&h<=oldY+0.65)floor=Math.max(floor,h);}
  if(p.y<=floor){p.y=floor;p.vy=0;p.grounded=true;}
  p.x=clamp(p.x,-MAP_SIZE/2+RADIUS,MAP_SIZE/2-RADIUS);p.z=clamp(p.z,-MAP_SIZE/2+RADIUS,MAP_SIZE/2-RADIUS);
}
