...
export interface MapBox { x: number; y: number; z: number; w: number; h: number; d: number; color: number; kind?: 'building' | 'crate' | 'wall' | 'cover' | 'platform'; }
export interface Ramp { x: number; z: number; w: number; d: number; h: number; axis: 'x' | 'z'; sign: number; color: number }
...
export const BOXES: MapBox[] = [
  ...
  {x:-4.7,y:1.77,z:-9,w:0.6,h:3.54,d:8,color:light,kind:'wall'},
  {x:4.7,y:1.77,z:-9,w:0.6,h:3.54,d:8,color:light,kind:'wall'},
...
export const SPAWNS: (Vec3 & {yaw: number})[] = [
...
export function rampHeight(r: Ramp, x: number, z: number): number | null {
  if (Math.abs(x-r.x)>r.w/2 || Math.abs(z-r.z)>r.d/2) return null;
