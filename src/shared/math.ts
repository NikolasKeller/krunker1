...
export function rayBox(o: Vec3,d: Vec3,min: Vec3,max: Vec3): number | null {
  ...
  for (const a of ['x','y','z'] as const) {
    if(Math.abs(d[a])<1e-9) { if(o[a]<min[a] || o[a]>max[a]) return null; }
    else { let a0=(min[a]-o[a])/d[a],a1=(max[a]-o[a])/d[a]; if(a0>a1) [a0,a1]=[a1,a0]; near=Math.max(near,a0); far=Math.min(far,a1); if(near>far) return null; }
  }
  return near;
}
...
export function rayRamp(o: Vec3,d: Vec3,r: typeof RAMPS[number]): number | null {
  ...
  const planes: [Vec3,number][] = [[{x:1,y:0,z:0},r.x+r.w/2],[{x:-1,y:0,z:0},-r.x+r.w/2],[{x:0,y:0,z:1},r.z+r.d/2],[{x:0,y:0,z:-1},-r.z+r.d/2],[{x:0,y:-1,z:0},0],[n,r.h/2-slope*(r.axis==='x'?r.x:r.z)]];
  let near=0,far=Infinity;
  for(const [p,b] of planes) { const v=p.x*d.x+p.y*d.y+p.z*d.z, q=b-p.x*o.x-p.y*o.y-p.z*o.z; if(Math.abs(v)<1e-9) {if(q<0)return null;} else if(v<0)near=Math.max(near,q/v);else far=Math.min(far,q/v); if(near>far)return null; }
  return near;
}
export function worldHit(o: Vec3,d: Vec3,range=150): number {
  let best=range;
  for(const b of BOXES) {const t=rayBox(o,d,{x:b.x-b.w/2,y:b.y-b.h/2,z:b.z-b.d/2},{x:b.x+b.w/2,y:b.y+b.h/2,z:b.z+b.d/2}); if(t!==null&&t<best)best=t;}
  for(const r of RAMPS) {const t=rayRamp(o,d,r);if(t!==null&&t<best)best=t;}
...
export function hitPlayer(o: Vec3,d: Vec3,p: Pick<PlayerState,'x'|'y'|'z'|'slide'>) {
  ...
  let best: {zone:'head'|'body'|'legs';distance:number}|null=null;
  for(const z of zones) {const t=rayBox(o,d,{x:p.x-z.r,y:p.y+z.lo*crouch,z:p.z-z.r},{x:p.x+z.r,y:p.y+z.hi*crouch,z:p.z+z.r});if(t!==null&&(!best||t<best.distance))best={zone:z.zone,distance:t};}
  return best;
}
