import { CLASS_IDS, CLASSES } from '../shared/weapons';
import type { WeaponId } from '../shared/types';

export function homeMarkup(icon: (id: WeaponId) => string) {
    return `<main id="home" class="home-screen" aria-label="Furo home">
      ...
      <section class="home-controls" aria-labelledby="home-title">
        ...
        <section class="home-loadout" aria-label="Choose your class"><div class="picker-heading"><span>YOUR CLASS</span><small>CHOOSE YOUR PLAYSTYLE</small></div><div class="class-cards">${CLASS_IDS.map((id, i) => `<button class="class-card" data-home-class="${id}" style="--class-color:${CLASSES[id].color}" aria-pressed="false"><span class="card-number">0${i + 1}</span><span class="card-check">✓</span><div class="card-weapon">${icon(CLASSES[id].weapon)}</div><strong>${CLASSES[id].name}</strong><small>${CLASSES[id].role}</small></button>`).join('')}</div><p id="home-class-description" aria-live="polite"></p></section>
