...
export type TouchRole = 'move' | 'look' | 'fire' | 'jump' | 'slide' | 'reload';
...
export class TouchInput {
    ...
    begin(id: number, role: TouchRole, x: number, y: number) {
        ...
        if (['fire', 'jump', 'slide', 'reload'].includes(role)) this.taps.add(role);
        return true;
    }
    ...
    consumed(input?: { jump: boolean; slide: boolean; reload: boolean }) {
        for (const role of ['jump', 'slide', 'reload'] as const) if (!input || input[role]) this.taps.delete(role);
    }
