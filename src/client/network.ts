...
export class Network {
  ...
  players=new Map<string,PlayerState>();predicted?:PlayerState;round?:RoundState;difficulty:Difficulty='normal';bots=5;
  pending:Input[]=[];outgoing:Input[]=[];frames:{time:number;players:Map<string,PlayerState>}[]=[];
  correction={x:0,y:0,z:0};onEvents:(events:GameEvent[])=>void=()=>{};onNotice:(text:string)=>void=()=>{};onWelcome:()=>void=()=>{};
  lastSnapshot=0;receivedAt=0;bytes=0;reconciliations=0;maxCorrection=0;
  private retry=0;private generation=0;private reconnectTimer?:ReturnType<typeof setTimeout>;private config?:{name:string;room:string;classId:ClassId;team:Team};
  constructor(){setInterval(()=>{this.send({type:'ping',time:Date.now()});if(this.ws?.readyState===WebSocket.OPEN&&Date.now()-this.receivedAt>6000&&this.receivedAt>0)this.status='CONNECTION STALLED';},1500);}
  connect(config:{name:string;room:string;classId:ClassId;team:Team}) {
    this.config=config;const generation=++this.generation;clearTimeout(this.reconnectTimer);this.ws?.close();
    this.status='CONNECTING';this.players.clear();this.frames=[];this.predicted=undefined;this.pending=[];this.outgoing=[];this.lastSnapshot=0;
    const ws=new WebSocket(`${location.protocol==='https:'?'wss':'ws'}://${location.host}/ws`);this.ws=ws;
    ws.onopen=()=>{if(generation!==this.generation)return;this.status='CONNECTED';this.retry=0;this.send({type:'join',...config,token:sessionStorage.getItem(`arena-token-${config.room}`)??undefined});this.send({type:'ping',time:Date.now()});};
    ws.onmessage=e=>{if(generation!==this.generation)return;try{this.bytes+=e.data.length;this.receive(JSON.parse(e.data));}catch(error){console.error('Invalid server message',error);}};
    ws.onclose=e=>{if(generation!==this.generation)return;this.status=e.code===4000?'SESSION MOVED':'RECONNECTING';if(e.code!==4000)this.reconnectTimer=setTimeout(()=>this.connect(config),Math.min(5000,600*2**this.retry++));};
  ...
  private receive(m:ServerMessage){
    ...
    if(m.type==='error'){this.status=m.message;this.onNotice(m.message);}
    if(m.type==='events')this.onEvents(m.events);
    if(m.type==='snapshot'){
      if(!m.full&&m.base!==this.lastSnapshot){this.send({type:'sync'});return;}
      this.receivedAt=Date.now();this.status='CONNECTED';this.lastSnapshot=m.n;this.host=m.host;this.round=m.round;this.difficulty=m.difficulty;this.bots=m.bots;
  ...
  remotePlayers():PlayerState[]{
    const time=this.serverNow-INTERPOLATION_MS;let a=this.frames[0],b=a;if(!a)return [];
    for(const f of this.frames){b=f;if(f.time>=time)break;a=f;}
    const t=clamp((time-a.time)/(b.time-a.time||1),0,1);
    return [...b.players.values()].filter(p=>p.id!==this.id).map(q=>{const p=a.players.get(q.id);if(!p||p.life!==q.life)return q;return {...q,x:lerp(p.x,q.x,t),y:lerp(p.y,q.y,t),z:lerp(p.z,q.z,t),yaw:angleLerp(p.yaw,q.yaw,t),pitch:lerp(p.pitch,q.pitch,t)};});
  }
}
