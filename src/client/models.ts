...
export function makeGun(id:WeaponId):THREE.Group {
  ...
  if(!shotgun){box(g,0,-0.235,-0.11,0.14,smg?0.36:0.29,0.19,dark).rotation.x=smg?0:-0.25;box(g,0,-0.35,-0.14,0.145,0.05,0.20,metal);}
  box(g,0,-0.15,0.02,0.15,0.035,0.19,black);box(g,0,-0.10,-0.075,0.15,0.12,0.035,black);
  box(g,0,0.015,-0.39,shotgun?0.23:0.17,0.18,0.40,shotgun?wood:sniper?stock:wood);
  const length=sniper?0.65:smg?0.19:0.43;
  if(shotgun){for(const x of [-0.072,0.072]){box(g,x,0.065,-0.75,0.095,0.095,0.53,metal);box(g,x,0.065,-1.02,0.072,0.07,0.01,black);}}
  else{box(g,0,0.045,-0.59-length/2,0.075,0.075,length,metal);box(g,0,0.045,-0.59-length,0.11,0.11,0.08,black);}
  if(sniper){
    box(g,0,0.17,-0.12,0.11,0.12,0.3,black);box(g,0,0.29,-0.09,0.18,0.18,0.61,black);
    for(const z of [-0.41,0.23]){const m=new THREE.Mesh(new THREE.CylinderGeometry(0.14,0.14,0.13,8),material(black));m.rotation.x=Math.PI/2;m.position.set(0,0.29,z);g.add(m);}
...
export function makeCharacter(classId:ClassId,color:number):Character {
  ...
  box(g,0,1.03,0,0.70,0.64,0.4,color);box(g,0,1.03,-0.226,0.51,0.47,0.07,vest);box(g,0,0.75,0,0.73,0.075,0.43,0x292d2a);
  for(const x of [-0.155,0.015,0.18])box(g,x,0.98,-0.285,0.125,0.23,0.05,0x5a6352);
  box(g,0,1.05,0.27,0.44,0.45,0.16,0x464b3d);
  const head=new THREE.Group();head.position.y=1.57;g.add(head);box(head,0,0,0,0.52,0.50,0.49,skin);
  box(head,0,0.23,0.035,0.57,0.16,0.54,classId==='hunter'?0x536347:color);box(head,0,0.17,-0.26,0.58,0.075,0.2,0x323b32);
  box(head,0,0.015,-0.254,0.48,0.115,0.035,0x20282a);box(head,0,-0.13,-0.25,0.50,0.18,0.04,0x343b38);
  const arms=new THREE.Group();arms.position.set(0,1.19,0);g.add(arms);
  const left=box(arms,-0.42,-0.14,-0.14,0.25,0.52,0.28,color);left.rotation.x=-0.8;left.rotation.z=-0.18;
