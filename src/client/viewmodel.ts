...
export class Viewmodel {
    ...
    setWeapon(id: WeaponId) { this.draw = SWITCH_MS / 1000; this.aim = 0; this.rig.remove(this.gun); this.gun = makeGun(id); this.rig.add(this.gun); this.weapon = id; this.kick = 0.08; this.flash.position.set(0, 0.045, id === 'sniper' ? -1.38 : id === 'shotgun' ? -1.18 : id === 'pistol' ? -0.5 : -1.01); }
    ...
    update(dt: number, time: number, speed: number, aiming: boolean, reloadEnd: number, now: number, slide: number) {
        ...
        this.rig.position.y -= .08 * this.draw / (SWITCH_MS / 1000);
