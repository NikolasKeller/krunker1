import { abilityMarkup, updateAbilityHUD } from './ability-hud';
...
export class UI {
    ...
    private spots?: Extract<GameEvent, { type: 'spot' }>;
    ...
    `;
        ...
        minimapSetting.onchange = () => {
            this.minimapEnabled = minimapSetting.value === 'on';
            localStorage.setItem('arena-minimap', minimapSetting.value);
            $('minimap').classList.toggle('hidden', !this.minimapEnabled);
        };
    ...
    event(e: GameEvent, renderer: Renderer, now: number) {
        if (e.type === 'spot' && e.viewer === this.net.id) this.spots = e;
        if (e.type === 'kill') {
    ...
    update(now: number, renderer: Renderer, aiming: boolean, remotes = this.net.remotePlayers()) {
        ...
        if (p) {
            updateAbilityHUD(p, serverNow, round?.phase === 'playing');
            const spots = this.spots;
            $('watchpoint-spots').innerHTML = spots && spots.life === p.life && p.alive && round?.phase === 'playing' && (p.abilityUntil ?? 0) > serverNow && spots.until > serverNow ? spots.points.map(pos => { const screen = renderer.project(pos); return screen.visible ? `<span style="left:${screen.x}px;top:${screen.y}px">◇<small>LAST SEEN</small></span>` : ''; }).join('') : '';
