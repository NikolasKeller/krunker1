...
export function startRound(r:RoundState,now:number) {Object.assign(r,{phase:'playing',endsAt:now+r.duration,round:r.round+1,blue:0,red:0,winner:'',nextAt:0});}
export function checkRound(r:RoundState,players:Iterable<PlayerState>,now:number):boolean {
  ...
  if(!limit&&now<r.endsAt)return false;
  r.phase='results';r.nextAt=now+12000;
  r.winner=r.mode==='tdm'?(r.blue===r.red?'DRAW':r.blue>r.red?'BLUE TEAM':'RED TEAM'):(list[0]?.name??'DRAW');return true;
}
