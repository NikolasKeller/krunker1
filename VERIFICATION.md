...
Node DOM tests observed **zero mutations over 100 unchanged polls**, including unrelated player-position changes. They retain the button, its text target, every form control, and existing roster/results rows; callsign focus, draft value, selection direction and caret survive readiness/team/countdown updates. These use jsdom, not a browser. The build and **50 tests** passed, including all 39 original tests, seven ready/countdown state-machine regressions, and four DOM/action regressions.

Ready/create actions now run directly in the UI, with no audio/renderer/pointer-lock prerequisite. Game key handling also leaves Space available for native button activation. The existing server ready handler and snapshot replication passed before these changes; the dropped-click fix is in the client interaction path.

`npm run test:lobby:flow` passed against its own real server and, with `GAME_URL=http://127.0.0.1:8080`, the running production server. Each of two independent Node processes uses its own DOM, storage, actual UI/Network code and WebSocket. Clicking CREATE/JOIN and READY produced one outgoing ready message per client, authoritative **1/2 then 2/2 ready** snapshots at both clients, the same countdown deadline and actual `playing` states. The owned-server test also inspected the server's ready flags. The production run measured **10 timer polls in one second and zero idle DOM writes**, without a render loop.

Both runs also passed START EARLY with **0/2 ready**, nonhost-start rejection, stable controls through room switching and match start, duplicate callsigns, team switching and host migration. The broader `test:lobby` passed late joins with spawn protection, reconnects, results returning to the same lobby, rematch reset, countdown cancellation, and capacity/replacement checks. Browser pointer actionability and visual stability remain for the external browser recheck; jsdom clicks do not measure layout or hit testing.

After the fix, `test:production` passed compiled asset serving on port 8080, movement to a firing lane, elimination and respawn (60 Hz; 0.192 ms mean / 0.572 ms peak tick in its final sample). `test:integration` passed all primaries, historical hitscan, malformed-input rejection, results/lobby/rematch and reconnect. `test:lifecycle` passed disconnected-identity expiry and empty-room cleanup. No browser or CDP check was attempted.
