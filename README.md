...
`npm run preview:hud` exports standalone `artifacts/hud-preview/{lobby,ffa,tdm,headshot,multikill}.html` pages using the actual UI markup, stylesheet and embedded font. The combat fixtures freeze the yellow damage animation 150 ms into its lifetime and display the hitmarker and kill notice. They use a software geometry backdrop and require an external browser to verify layout; they do not connect to or change a live match. Capture them at 1024×614 to compare with the supplied reference, then check the deployed lobby and match at 1280×800 for clipping.
